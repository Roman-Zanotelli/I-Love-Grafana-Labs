...
fn _decode_key() -> DecodingKey{
    DecodingKey::from_secret(std::env::var("JWT_SECRET").unwrap_or("Default Secret".to_string()).as_bytes())
}
fn _encode_key() -> EncodingKey{
    EncodingKey::from_secret(std::env::var("JWT_SECRET").unwrap_or("Default Secret".to_string()).as_bytes())
}
