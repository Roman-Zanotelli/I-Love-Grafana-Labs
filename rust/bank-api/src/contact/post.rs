...
pub(super) async fn add(contact_id: &str, user_id: &str, pool: &Pool<Postgres>) -> Result<ContactResponse, sqlx::Error>{
    const QUERY: &str = r#"
        ...
        "#;
    internal_query(contact_id, user_id, pool, QUERY).await
}
...
async fn internal_query(contact_id: &str, user_id: &str, pool: &Pool<Postgres>, query: &str)-> Result<ContactResponse, sqlx::Error>{
    Ok(ContactResponse{
        contacts: Some(query_as::<_, Contact>(query).bind(user_id).bind(contact_id).fetch_all(pool).await?),
    })
}